    ...
    troubleshooting: [
      {
        ...
        problem:
          "랭킹 페이지와 마이페이지에서 '이전 시즌 랭킹'이라는 동일한 성격의 데이터를 호출해야 했습니다. 하지만 랭킹은 누구나 볼 수 있는 요약 정보이고, 마이페이지는 본인만 보는 상세 투자 내역이 포함되어야 해서 불필요한 데이터 페칭(Fetching)과 렌더링 지연이 발생했습니다.",
        solution:
          "백엔드 팀과 조율하여 목적에 맞게 두 개의 DTO(데이터 전송 객체)로 분리해줄 것을 요청하고 적용했습니다. 이를 통해 프론트엔드 단에서 불필요한 데이터를 필터링하는 과정을 생략하여 렌더링 효율과 보안을 동시에 높였습니다.",
      },
      {
        title: "6인 협업 환경에서의 CSS 스타일 충돌 해결",
        problem:
          "6명이 동시에 각자의 페이지를 개발하다 보니 사전에 클래스명 네이밍 규칙을 정했음에도 병합(Merge) 과정에서 글로벌 스타일 충돌과 UI 파편화가 발생했습니다.",
        solution:
